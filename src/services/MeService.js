...
const MeService={
    ...
    async getNumberCommonGroup(_id,senderId){
        const num = await Conversation.aggregate([
            { 
                $match:{"members.userId":{$all:[ObjectId(_id),
                ObjectId(senderId)]}}
        ...
        let listFriendSender =await Friend.aggregate([
            {
                ...
                ,
                {
                  $match:{"userId":{$ne:ObjectId(senderId)}}
                }
